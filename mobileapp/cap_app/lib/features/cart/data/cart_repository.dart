// File purpose: Connects Flutter cart feature code to backend or local data sources.
...
class CartRepository {
  ...
  Future<CartComparisonResponse> compareSingleSupermarket(
    List<CartCompareItemRequest> items,
  ) async {
    final raw = await _apiClient.post(
      '/api/v1/cart/compare/single-supermarket',
      data: {'items': items.map((item) => item.toJson()).toList()},
    );
    return CartComparisonResponse.fromJson(
